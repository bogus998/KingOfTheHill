...
enum DieFace { ONE = 1, TWO = 2, THREE = 3, GOLD = 4, CLAW = 5, HEART = 6 }

static func resolve(faces: Array) -> Dictionary:
	var result := { "gems": 0, "gold": 0, "claws": 0, "hearts": 0 }
	var counts := {}
	for f in faces:
	...
	for num in [DieFace.ONE, DieFace.TWO, DieFace.THREE]:
		...
		if count >= 3:
			result["gems"] += num + (count - 3)
	result["gold"] = counts.get(DieFace.GOLD, 0)
