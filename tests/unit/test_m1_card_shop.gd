...
func test_purchase_deducts_gold_cost() -> void:
	var cost := CardShop.visible_cards[0].gold_cost
	CardShop.purchase(0, 0)
	assert_eq(PlayerManager.players[0].gold, 10 - cost)
...
func test_purchase_returns_false_when_insufficient_gold() -> void:
...
func test_refresh_costs_2_gold() -> void:
	CardShop.refresh_pool(0)
	assert_eq(PlayerManager.players[0].gold, 8)

func test_refresh_returns_true_on_success() -> void:
	assert_true(CardShop.refresh_pool(0))

func test_refresh_returns_false_when_insufficient_gold() -> void:
